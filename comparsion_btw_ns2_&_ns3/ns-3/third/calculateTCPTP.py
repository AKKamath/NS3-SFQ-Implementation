...
for file_name in sys.argv[1:]:
    os.system("sudo tshark -r " + file_name +" -T fields -e frame.time_relative -e frame.len -e ip.dst tcp.port == 50000  >" + file_name[0:file_name.rindex('.')] + ".csv")
    f = open( 'TP-' + file_name[0:file_name.rindex('.')] + 'TCP.plotme', 'w')
    old_time = 0
    new_time =0
    val = 0
    p = 0.0
