/*
 * This script simulates light TCP traffic for Sfq evaluation
...
{
  uint32_t qSize = StaticCast<SfqQueueDisc> (queue)->GetNBytes ();
...
//  LogComponentEnable ("SfqQueueDisc", LOG_LEVEL_INFO);
 ...
 // Config::SetDefault ("ns3::PfifoFastQueueDisc::Limit", UintegerValue (50));
  ...
  QueueDiscContainer queueDiscs = tchSfq.Install (devices_gateway);
  ...
  Ipv4InterfaceContainer udSfqnterfaces[2];
  ...
  for (int i = 0; i < 2; i++)
    {
      ...
      address.NewNetwork ();
      udSfqnterfaces[i] = address.Assign (udpdevices[i]);
    }
  //devices_gateway = bottleneckLink.Install (gateway.Get (0), gateway.Get (1));
  address.NewNetwork ();
  ...
  if (writeForPlot)
    {
      filePlotQueue << pathOut << "/" << "Sfq-queue3.plotme";
  ...
  if (printSfqStats)
    {
      ...
      std::cout << "*** Sfq stats from bottleneck queue ***" << std::endl;
